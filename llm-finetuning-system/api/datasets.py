...
try:
    ...
    list_function = deployed_app.list_models_and_datasets if hasattr(
        deployed_app, 'list_models_and_datasets') else None
...
def list_huggingface_models(query: Optional[str] = None, limit: int = 10) -> List[dict]:
    """
    List HuggingFace models - function that can be mocked in tests
    """
...
def search_models(query: str) -> List[dict]:
    """
    Search models - function that can be mocked in tests
    """
    # Mock search results
    return [
        {
            "id": f"model-{query}-1",
            "name": f"Model for {query}",
            "description": f"Model related to {query}",
            "downloads": 1000
        }
    ]
...
def get_model_info(model_id: str) -> dict:
    """
    Get model info - function that can be mocked in tests
    """
    return {
        "id": model_id,
                ...
                for i, line in enumerate(lines):
                    try:
                        ...
                        if not isinstance(data, dict):
                            errors.append(
                                f"Line {i+1}: Expected JSON object, got {type(data).__name__}")
                        else:
                            valid_lines += 1
...
async def list_datasets(
    source: Optional[str] = Query(
        None, description="Filter by source (upload, huggingface, modal)"),
    type: Optional[DatasetType] = Query(
        None, description="Filter by dataset type"),
    limit: int = Query(
        50, ge=1, le=200, description="Maximum number of datasets to return"),
            ...
            for ds in modal_datasets:
                dataset_info = DatasetInfo(
                    ...
                    created=datetime.fromtimestamp(
                        ds.get("created", time.time())),
                    modified=datetime.fromtimestamp(
                        ds.get("modified", time.time())),
                    source="modal"
                )
...
async def upload_dataset(
    ...
    name: Optional[str] = Form(
        None, description="Custom name for the dataset"),
...
async def preview_dataset(
    ...
    try:
        ...
        if dataset.type == DatasetType.JSONL:
            ...
            with open(dataset.path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
                preview_data = [{"line": i+1, "content": line.strip()}
...
async def list_available_models(
    source: Optional[ModelSource] = Query(
        None, description="Filter by model source"),
    limit: int = Query(
        50, ge=1, le=200, description="Maximum number of models to return"),
    search: Optional[str] = Query(
        None, description="Search models by name or description")
):
    """
