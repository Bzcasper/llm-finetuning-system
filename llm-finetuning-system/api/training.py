...
class TrainingConfig(BaseModel):
    ...
    model_name: str = Field(...,
                            description="Model name or HuggingFace model path")
    dataset_path: str = Field(..., description="Path to training dataset")
    output_dir: str = Field(default="/vol/finetuned_model",
    ...
    lora_dropout: float = Field(
        default=0.05, ge=0.0, le=1.0, description="LoRA dropout rate")

    # Training Parameters
    learning_rate: float = Field(
        default=0.0002, gt=0.0, le=1.0, description="Learning rate")
    num_train_epochs: int = Field(
        default=3, ge=1, le=100, description="Number of training epochs")
    per_device_train_batch_size: int = Field(
        default=2, ge=1, le=64, description="Batch size per device")
    gradient_accumulation_steps: int = Field(
        default=1, ge=1, le=128, description="Gradient accumulation steps")
    max_seq_length: int = Field(
        default=512, ge=128, le=4096, description="Maximum sequence length")
    ...
    warmup_ratio: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Warmup ratio")
    lr_scheduler_type: str = Field(
        default="cosine", description="Learning rate scheduler")

    # Quantization and Hardware
    use_4bit_quantization: bool = Field(
        default=True, description="Use 4-bit quantization (QLoRA)")
    use_8bit_quantization: bool = Field(
        default=False, description="Use 8-bit quantization")
    gpu_type: GPUType = Field(default=GPUType.A100,
                              description="GPU type for training")
    timeout: int = Field(default=3600, ge=300, le=86400,
                         description="Training timeout in seconds")

    # Advanced Settings
    save_steps: int = Field(
        default=500, ge=1, description="Save checkpoint every N steps")
    eval_steps: int = Field(default=500, ge=1, description="Evaluation steps")
    logging_steps: int = Field(
        default=10, ge=1, description="Logging frequency")
    seed: int = Field(default=42, description="Random seed")

    # Data Processing
    train_on_inputs: bool = Field(
        default=False, description="Train on input tokens")
    group_by_length: bool = Field(
        default=True, description="Group sequences by length")
...
try:
    deployed_app = modal.App.lookup(
        "llm-finetuner", environment_name="ai-tool-pool")
...
def start_training_job(config: dict) -> dict:
    """
    Start a training job - function that can be mocked in tests
    This is used by tests for patching
    """
...
def stop_training_job(job_id: str) -> dict:
    """
    Stop a training job - function that can be mocked in tests
    This is used by tests for patching
    """
...
async def get_training_logs(
    ...
    if job_id not in training_logs:
        raise HTTPException(
            status_code=404, detail="Training job logs not found")
...
async def get_training_metrics(
    job_id: str = Path(..., description="Training job ID"),
    window: int = Query(60, ge=10, le=3600,
                        description="Time window in seconds for metrics")
):
    """
    ...
    if metrics_timeline:
        ...
        avg_loss = sum(m.get("loss", 0) for m in metrics_timeline if m.get(
            "loss")) / len([m for m in metrics_timeline if m.get("loss")])
    else:
        latest_metrics = {}
        avg_loss = None
...
async def run_training_simulation(config: dict, job_id: str):
    ...
    monitor.log(
        "Starting training simulation (Modal.com not connected)", level="INFO")
    monitor.update_status(JobStatus.RUNNING)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        loss = max(0.1, 2.5 - (epoch * 0.3) +
                   (0.1 * (0.5 - 0.5)))  # Add some randomness
        ...
        monitor.update_progress(epoch, total_epochs,
                                epoch * 100, total_epochs * 100, metrics)
        monitor.log(
