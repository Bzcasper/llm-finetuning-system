...
export default function RestTestPage() {
  ...
  const generateImage = async () => {
    ...
    try {
      // Use our server-side API endpoint with the REST approach
      addLog("Sending request to server-side REST API")
      const response = await fetch("/api/rest-generate", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="container max-w-4xl py-8">
      <h1 className="text-2xl font-bold mb-4">REST API Fal AI Test</h1>
      <p className="text-gray-500 mb-6">This test uses the REST API approach to generate an image with Fal AI.</p>

      <div className="space-y-6">
        <div className="border p-4 rounded-lg">
          <h2 className="text-lg font-medium mb-2">Generate an Image</h2>
          <p className="text-sm text-gray-500 mb-4">
            This will generate an image of "a beautiful house with a garden" using the fast-sdxl model.
          </p>
